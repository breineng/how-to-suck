...
namespace HowToSuck.Audio
{
    ...
    public sealed class AudioBindings:ScriptableObject
    {
        ...
        public bool Validate(out string error)
        {
            ...
            int count=Enum.GetValues(typeof(SfxId)).Length;var seen=new bool[count];
            foreach(var entry in Entries)
            {
                if(entry==null||(int)entry.Id>=count||!Enum.IsDefined(typeof(ClipReviewStatus),entry.Status)||!Enum.IsDefined(typeof(AudioBus),entry.Bus)||entry.Bus==AudioBus.Master||seen[(int)entry.Id]){error="Audio IDs must occur exactly once.";return false;}
            ...
            foreach(bool present in seen)if(!present){error="All sound IDs including pending entries are required.";return false;}
