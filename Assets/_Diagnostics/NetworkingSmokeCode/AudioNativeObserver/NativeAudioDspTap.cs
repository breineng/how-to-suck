...
{
    public sealed class NativeAudioCaptureData
    ...
    public sealed class NativeAudioCaptureBuffer
    {
        ...
        {
            if(sampleRate!=48000||seconds<1||seconds>120)throw new ArgumentException("Bounded 48kHz stereo capture required.");
        ...
        public NativeAudioCaptureData Drain()
        {
            lock(gate)
            {
                // Acquiring this lock after armed=false is the complete callback drain handshake.
                armed=false;var value=new NativeAudioCaptureData{Samples=samples,Count=count,Callbacks=callbacks,Rate=rate,Channels=2,Fault=fault};
                samples=null;count=callbacks=0;fault=null;return value;
            }
        }
    }
    public sealed class NativeAudioDspTap:MonoBehaviour
    {
        public readonly NativeAudioCaptureBuffer Buffer=new NativeAudioCaptureBuffer();
    ...
    [Serializable] public sealed class NativeAudioCaptureReceipt
    {
    ...
    public static class NativeAudioCaptureFile
    {
        // Runs on the diagnostic main thread only after Drain. IEEE float preserves peaks; no normalization/clamping.
        public static NativeAudioCaptureReceipt Write(string path,NativeAudioCaptureData data)
        {
            ...
            return new NativeAudioCaptureReceipt{path=path,sha256=hash,fault=data.Fault,rate=data.Rate,channels=data.Channels,callbacks=data.Callbacks,
